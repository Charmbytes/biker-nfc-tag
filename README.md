...
1. A rider fills a one-time form: name, age, blood group, allergies, medical history, surgeries, a photo, and an emergency contact.
...
4. Anyone at an accident scene taps or scans it — the rider's medical ID card opens in any browser, showing their photo and details, with a **one-tap CALL button** to reach family immediately.
...
- **Privacy by design** — riders choose exactly what to share; nothing is collected beyond what they type, and sensitive fields like a government ID are explicitly optional.
...
✅ Photo and optional government ID upload, embedded directly in the record
...
| Phase | Milestone |
|-------|-----------|
| Now | Web prototype with QR + NFC-ready links, photo upload |
| Next | Editable records with permanent IDs, short-link/redirect support so photos fit in printed QR codes, multilingual support, printed sticker + NFC tag kits |
...
- Photos are compressed and embedded client-side; QR codes fall back to a photo-free version when a record is too large to fit (photos still travel with the full link)
- QR codes generated client-side; call button uses the native `tel:` protocol
- Planned v2: lightweight backend (Node.js + SQLite/Postgres) for editable records, permanent short links, and QR codes that carry photos via redirect
